...
"""
Asana Prioridades Agent - Hórus System
Agente focado exclusivamente em baixar dados de Gestão de Prioridades e Custos do Asana e sincronizar com o PostgreSQL (NocoDB).
"""
...
PROJECT_PRIORIDADES = os.getenv('ASANA_PROJECT_PRIORIDADES', '1212946699570594')
...
def get_headers():
    return {
        ...
        'User-Agent': 'AsanaPrioridadesAgentPython/1.0'
    }
...
# --- AÇÃO: BAIXAR DADOS ---
def baixar_dados():
    """Baixa os JSONs brutos do projeto de Prioridades e salva na pasta TempAssana"""
    print("[*] Iniciando download dos dados de Gestão de Prioridades do Asana...")
    ...
    url = f'https://app.asana.com/api/1.0/projects/{PROJECT_PRIORIDADES}/tasks?opt_expand={fields}'
    
    # Rotação de cache/arquivo anterior para exclusão segmentada inteligente
    filepath = os.path.join(TEMP_DIR, PRIORIDADES_FILE_NAME)
    cachepath = os.path.join(TEMP_DIR, PRIORIDADES_CACHE_NAME)
    ...
    print(f"[*] Buscando tarefas do projeto Gestão de Prioridades: {url}")
    ...
    if dados and 'data' in dados:
        ...
        print("[-] Falha ao baixar dados de Gestão de Prioridades.")
        if os.path.exists(cachepath):
...
def process_task_tree(task, context, level, results):
    ...
    if level == 1:
        ...
        if num_subtasks == 0:
            ...
            if current_context.get('kr'):
                ...
                current_context['projects'] = '📦 Prioridades Gerais'
    ...
    if task_projects:
        for proj in task_projects:
            ...
            if 'OKR' in proj_name_upper or proj_id == '1212434383653814':
                esta_okr_flag = True
            if 'PRIORIDADE' in proj_name_upper or 'INOVAÇÃO' in proj_name_upper or proj_id == PROJECT_PRIORIDADES:
                esta_priori_flag = True
                
    # Usaremos PROJECT_PRIORIDADES como referência direta no código
    esta_priori_flag = esta_priori_flag or (task_projects and any(proj.get('gid') == PROJECT_PRIORIDADES for proj in task_projects))
...
def sincronizar_banco():
    """Lê os arquivos de Prioridades baixados, desduplica, associa inteligentemente e popula o PostgreSQL"""
    print("[*] Iniciando leitura dos arquivos de Gestão de Prioridades e sincronização...")
    
    priori_file = os.path.join(TEMP_DIR, PRIORIDADES_FILE_NAME)
    cache_file = os.path.join(TEMP_DIR, PRIORIDADES_CACHE_NAME)
    
    if not os.path.exists(priori_file):
        print("[-] Erro: Arquivo local de Gestão de Prioridades não encontrado. Execute '--action baixar' primeiro.")
        return False
        
    results = {}
    
    print("[*] Processando tarefas de Gestão de Prioridades...")
    with open(priori_file, 'r', encoding='utf-8') as f:
        priori_data = json.load(f)
        for root_task in priori_data.get('data', []):
    ...
    print(f"[+] Total de cards únicos de Prioridades processados: {len(results)}")
    ...
    try:
        ...
        if ids_para_deletar:
            print("[*] Executando deleção segmentada dos registros anteriores de Prioridades...")
        ...
        print("[*] Gravando novos dados de Prioridades no PostgreSQL...")
        sql = """
        ...
        conn.commit()
        print(f"[+] Sincronização de Prioridades concluída: {inserted} registros inseridos, {errors} erros.")
        cur.close()
        return True
    except Exception as e:
...
# --- MÉTODOS CLI ---
def main():
    parser = argparse.ArgumentParser(description="Asana Prioridades Agent - Hórus System")
